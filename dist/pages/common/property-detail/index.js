...
const _sfc_main = /* @__PURE__ */ common_vendor.defineComponent({
  ...
  setup(__props) {
    ...
    const imageUrls = common_vendor.computed(() => {
      ...
      if ((_a = detail.value.images) == null ? void 0 : _a.length) {
        return detail.value.images.map((img) => api_file.getPreviewUrl(img.id));
      }
      if (!detail.value.coverImage) return [];
      const url = detail.value.coverImage.startsWith("http") ? detail.value.coverImage : `${api_http.BASE_URL}/${detail.value.coverImage}`;
      return [url];
    });
