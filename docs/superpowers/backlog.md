...
## 5. 角色模式门控（2026-06-17 已落地，遗留项）

- 用户端视觉打磨到生产级（找房/地图/我的用户视图卡片、空态、动效）——延后。
- 管理端页面移入分包（subpackage）减小主包——延后（当前主包 631KB，未超限）。
- 私有图片匿名浏览：用户端未登录时，走 downloadFile(Bearer) 的私有图可能不显示；如需匿名也能看图，要给 sysFile/Preview 加匿名放行（涉及 Core 系统服务，谨慎）。
- 删除未使用的 `src/components/sl-property-filter`（独立版，已被 sl-property-filter-bar 取代）——清理项。
