...
let mediaTrace = createMediaTrace('property')
  ...
  try {
    for (const item of files) {
      try {
        ...
        console.error('upload property media failed', error)
      }
      finally {
...
function handleChooseMediaFailure(error: unknown, allowFallback = true) {
  ...
  console.error('choose property media failed', error)
  ...
  console.error('fallback property media picker failed', mediaPickerErrorText(error))
...
function chooseMedia() {
  if (uploading.value)
    return
  mediaTrace = createMediaTrace('property')
...
<template>
  <view class="sl-page form-page">
            ...
            >
              ...
              <view v-else class="video-tile">
                ...
                <video
                  ...
                  class="video-poster video-poster--local"
                  :src="media.url"
                  :controls="false"
    ...
    <view class="bottom-bar sl-safe-bottom">
      ...
      <wd-button block type="primary" :loading="submitting" :disabled="uploading" @click="submit">
        {{ isEdit ? '保存修改' : '发布房源' }}
      </wd-button>
    </view>
  </view>
