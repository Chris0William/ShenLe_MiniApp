...
describe('business discovery experience', () => {
  it('requests only available supply and shows business context without landlord media', () => {
    ...
    expect(map).toContain('<sl-map-community-card')
    expect(map).toContain('<sl-landlord-map-card')
  ...
  it('renders every pet status through the shared styled component below commission', () => {
    ...
    const businessCard = source('src/components/sl-map-community-card/sl-map-community-card.vue')

    expect(map).toContain('sl-map-community-card')
    expect(properties).toContain('<sl-pet-policy-text')
    expect(rooms).toContain('<sl-pet-policy-text')
    // Pet and elevator tags moved above fees in the redesigned map card attribute row.
    expect(businessCard.indexOf('attr-tag--pet')).toBeLessThan(businessCard.indexOf('bmapcard__commission'))
    expect(businessCard).toContain("elevatorMode === 2 ? '楼梯' : '电梯'")
